...
const AddTokens = ( {users=[], tokensNumber, address, selectedUsers, setSelectedUsers, setAddress, setTokens}:Props) => (
    <StyledAddTokens>
        ...
        <div className='add-tokens__value-container'>
            ...
            <input placeholder='Type value' value={tokensNumber || ''} onChange={e => setTokens(parseInt(e.target.value, 10))} className='add-tokens__value-input' type='text' />
      </div>

      <div className='add-tokens__address-container'>
            <span className='add-tokens__label'>Address</span>
            <input placeholder='Type address' value={address} onChange={e => setAddress(e.target.value)} className='add-tokens__value-input' type='text' />
      </div>
    </StyledAddTokens>
)
